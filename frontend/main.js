...
(() => {
    function mountPublicSocketConnection() {
        ...
        const publicSocket = io("https://formait-backend.onrender.com", { transports: ["websocket"] });
        ...
        tableContainer.addEventListener("click", async (e) => {
            ...
            try {
                ...
                document.getElementById("crmExecuteDeleteBtn").onclick = async () => {
                    ...
                    try {
                        await fetch(`https://formait-backend.onrender.com/api/${routePath}/${lead._id}`, { method: "DELETE", headers });
                        targetRow.remove();
                        closeModalWindow();
                ...
                if (modalReviewBtn && !isProcessed) {
                    modalReviewBtn.onclick = async () => {
                        const routePath = isSupportMessage ? 'messages' : 'leads';
                        try {
                            await fetch(`https://formait-backend.onrender.com/api/${routePath}/${lead._id}`, {
...
async function loadSubmoduleContent(tabId) {
    ...
    try {
        ...
        if (tabId === "support" || tabId === "support center" || tabId === "messages") {
            ...
            const res = await fetch("https://formait-backend.onrender.com/api/messages", { headers });
...
async function triggerAgentLoop() {
    ...
    try {
        const response = await fetch('https://formait-backend.onrender.com/api/ai/execute-loop', {
            method: 'POST',
